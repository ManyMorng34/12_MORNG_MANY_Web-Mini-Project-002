        ...
        <aside className="w-64 bg-white shadow-md">
          <div className="w-64 p-4 text-gray-700 font-sans">
            <div className="mb-8">
              ...
              <div>
                {/* {workList.payload.map((item)=>)} */}
                {workList?.payload.map((work, index) => {
                  const randomColor =
                    colorValues[Math.floor(Math.random() * colorValues.length)];

                  return (
                    <ul key={index} className="mt-4 space-y-2">
                      <li className="flex items-center justify-between px-3 py-2 bg-gray-100 rounded-md cursor-pointer">
